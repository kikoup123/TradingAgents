...
class Adapter:
    ...
    def account_snapshot(self, account_alias: str) -> BrokerAccountSnapshot:
        return BrokerAccountSnapshot(account_alias=account_alias, broker_type=self.broker_type, broker_name="Broker", masked_account="••••1234", connected=True, currency="USD", balance=self._equity, equity=self._equity, used_margin=0.0, free_margin=self._equity)
...
def test_phase23_uses_each_accounts_current_equity_without_public_environment() -> None:
    intent = TradeIntent(trade_id="brokerage-accounts", canonical_symbol="NASDAQ", direction="BEARISH", entry_price=100.0, stop_price=110.0, target_price=80.0, selected_exit_mode="FULL_AT_SD_2")
